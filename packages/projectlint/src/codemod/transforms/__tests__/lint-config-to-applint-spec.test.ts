...
test('transform @ice/spec to @applint/spec', async () => {
  const fixtureDir = path.join(fixturesDir, 'ice-spec-config');
  const tmpFixtureDir = `${fixtureDir}-tmp`;
  ...
  try {
    ...
    const { devDependencies = {}, scripts = {} } = packageJSON;
    expect(Object.keys(devDependencies).includes('@applint/spec')).toBeTruthy();
    expect(Object.keys(devDependencies).includes('@ice/spec')).toBeFalsy();
...
test('transform empty project to @applint/spec', async () => {
  const fixtureDir = path.join(fixturesDir, 'no-spec-config');
  const tmpFixtureDir = `${fixtureDir}-tmp`;
